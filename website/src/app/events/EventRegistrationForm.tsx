...
export default function EventRegistrationForm({ 
  ...
  eventId: string;
  eventTitle: string;
  sendEventRegistration: (formData: FormData) => Promise<FormState> 
}) {
  ...
  const formActionWrapper = async (prevState: FormState, formData: FormData) => {
    // Add the eventId to the form data
    formData.append("eventId", eventId);
    
    const result = await sendEventRegistration(formData);
    if (result.success) {
      setSubmitted(true);
  ...
  if (submitted) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">Registration Request Received!</h3>
        <p className="text-gray-600 dark:text-gray-300 mb-3">
          Thank you for registering for {eventTitle}.
        </p>
        <p className="text-gray-600 dark:text-gray-300 mb-6">
          Your registration request is pending approval. We&apos;ve sent you a confirmation email, and you&apos;ll receive another email within 24 hours if your registration is approved.
        </p>
        <button
    ...
    <form ref={formRef} action={formAction} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          name="organization"
          className={`w-full px-4 py-2 border ${
            state.errors?.organization ? "border-red-500" : "border-gray-300 dark:border-gray-600"
          } rounded-lg focus:ring-red-500 focus:border-red-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white`}
          placeholder="Your Company"
          required
        />
        {state.errors?.organization && (
          <p className="mt-1 text-sm text-red-500">{state.errors.organization[0]}</p>
        )}
      </div>
